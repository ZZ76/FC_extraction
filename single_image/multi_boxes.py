...
def pickfromcenter(cx, cy):   # pick a series of images put into images[]
    ...
    for i in center_range:   #center_range
        ...
        if y2 >= height:
            y2 = height - 1
        color1 = colors_plasma[color_index]
        img = cv2.rectangle(img, (x1, y1), (x2, y2), color1, 2)
        color_index += 1
...
colors_plasma = colors_subselect(mpcm.plasma.colors, num_colors=7)   # 'viridis', 'plasma', 'inferno', 'magma', cubehelix
colors_viridis = colors_subselect(mpcm.viridis.colors, num_colors=20)

for i in range(0, len(colors_plasma)):
    temp = colors_plasma[i][0]
    colors_plasma[i][0] = colors_plasma[i][2]
    colors_plasma[i][2] = temp
#color1 = colors_plasma[random.randint(15, 19)]
#color2 = colors_plasma[random.randint(0, 5)]

bnr_image_name = 'DFC_AOI6_FC_tiles273.TIF'
image_name = 'DFC_AOI_RGB_tile273.TIF'
bin1 = cv2.imread(bnr_image_name, -1)
bin2 = cv2.imread(bnr_image_name, -1)
bin = cv2.cvtColor(bin1, cv2.COLOR_GRAY2BGR)
img = cv2.imread(image_name)
img2 = img.copy()
center_range = range(100, 200, 15)
...
for i in cnt:
    ...
    if x2 > width or y2 > height or (x2 - x) < 20:
        pass
    else:
        print x, y, x2, y2
        ...
        color1 = colors_viridis[random.randint(14, 19)]
